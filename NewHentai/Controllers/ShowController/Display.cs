...
namespace NewHentai.Controllers
{
    ...
    {
        public ActionResult Display(string key, string type, string lang, string page, string param)
        {
            if (page == "ALL")
            {
                page = "0";
            ...
            ViewData["MAX"] = dataMain.Rows.Count > 0 ?
                int.Parse(dataMain.Compute("MAX(PKEY)", "").ToString()) : Convert.ToInt32(Math.Pow(10, 9));
