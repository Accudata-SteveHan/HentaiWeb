...
{
    public class CrawDoujinshiController_old_1 : Controller
    {
        ...
        public CrawDoujinshiController_old_1()
        {
...
SELECT O.* ,
 ...
 ORDER BY STATUS , CONVERT(DATE,UPDATE_TIME) , PKEY DESC";
        ...
        public ActionResult GET_EXHENTAI_GALLERY()
        {
            ...
            JOB_WEB jobWeb = new JOB_WEB(site);
            jobWeb.authUser = Central.authUser;
            jobWeb.ProcessSite();

            #endregion Process_Site

            int pageCount = jobWeb.page_info.Count;
        ...
        public ActionResult GET_EXHENTAI_GALLERY_DETAIL(string param)
        {
            ...
            JOB_PAGE jobPage = new JOB_PAGE(pageInfo);
            jobPage.authUser = Central.authUser;
            jobPage.domain = basePage;
            jobPage.GetThreadList();
            jobPage.GetThreadList();

            List<THREAD_INFO> threads = jobPage.thread_info;
        ...
        public ActionResult GET_EXHENTAI_DATA_DETAIL(string param)
        {
            ...
            if (dataExHentai.Rows.Count == 0)
            {
                ...
                drExHentai["PKEY"] = pKey;
                drExHentai["SKEY"] = sKey;
                drExHentai["TYPE"] = bookInfo.type;
                drExHentai["RELEASE_TIME"] = bookInfo.upload_time;
                drExHentai["CREATE_TIME"] = DateTime.Now;
                drExHentai["UPDATE_TIME"] = DateTime.Now;
            ...
            if (dataExHentaiDetail.Rows.Count == 0)
            {
                ...
                jsonObject.Add("TYPE", "ADD");
                jsonObject.Add("PKEY", pKey);
                jsonObject.Add("SKEY", sKey);
            ...
            else
            {
                ...
                jsonObject.Add("TYPE", "EDIT");
                jsonObject.Add("PKEY", pKey);
                jsonObject.Add("SKEY", sKey);

            }

            drExHentaiDetail["NAME_ENG"] = bookInfo.eng_title;
            drExHentaiDetail["NAME_UNI"] = bookInfo.uni_title;
            drExHentaiDetail["PAGE"] = bookInfo.total_page;
            drExHentaiDetail["LANG"] = bookInfo.language;
            drExHentaiDetail["PIC"] = bookInfo.pic;
        ...
        public ActionResult GET_EXHENTAI_DATAALL_DETAIL(string param)
        {
            ...
            if (param == null)
            {
                ...
                jsonObject.Add("SEL", selRow);

                dataObject = this.GetObject(param);

                if (dataObject.Rows.Count == 0)
                {
                    jsonObject.Add("ERROR", "DATA EMPTY");
                    ViewData["DATA"] = jsonObject.ToString();
                    return View();
            ...
            try
            {
                ...
                if (dataExHentai.Rows.Count == 0)
                {
                    ...
                    drExHentai["SKEY"] = sKey;
                    drExHentai["TYPE"] = bookInfo.type;
                    drExHentai["LANG"] = bookInfo.language;
                    drExHentai["RELEASE_TIME"] = bookInfo.upload_time;
                    drExHentai["CREATE_TIME"] = DateTime.Now;
                    drExHentai["UPDATE_TIME"] = DateTime.Now;
                ...
                drExHentaiDetail["NAME_UNI"] = bookInfo.uni_title;
                drExHentaiDetail["PAGE"] = bookInfo.total_page;
                drExHentaiDetail["TYPE"] = bookInfo.type;
                drExHentaiDetail["LANG"] = bookInfo.language;
                drExHentaiDetail["PIC"] = bookInfo.pic;
                drExHentaiDetail["PIC_THUMB"] = (res["gmetadata"] as JArray)[0]["thumb"].ToString();
                drExHentaiDetail["USER_ID"] = "DATAALL";
                drExHentaiDetail["UPDATE_TIME"] = DateTime.Now;

                jsonObject.Add("NAME", bookInfo.uni_title != "" ? bookInfo.uni_title : bookInfo.eng_title);
                if (dataExHentaiDetail.Rows.Count == 0)
                {
                    dataExHentaiDetail.Rows.Add(drExHentaiDetail);
            ...
            catch (Exception ex)
            {
                ...
                jsonObject.Add("ERROR", string.Format("ID = {0} ERROR , Message = {1}", dataObject.Rows[selRow]["ID"] , ex.Message));
                ViewData["DATA"] = jsonObject.ToString();
