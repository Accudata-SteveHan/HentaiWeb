...
{
    public class CrawMangaController : Controller
    {
 ...
 ORDER BY STATUS , CONVERT(DATE,UPDATE_TIME) , PKEY DESC";
        ...
        public ActionResult GET_EXHENTAI_DATAALL_DETAIL(string param)
        {
            ...
            try
            {
                ...
                // This block restores the detailed processing and fallback logic that was previously removed.
                // It applies the API results (if present), then falls back to scraping the gallery page (thread) for missing fields,
                // and updates the DB tables accordingly. It mirrors the original behaviour prior to the accidental deletion.
                ...
                // Ensure detail row exists (possibly empty) so we can fill it
                if (dataExHentaiDetail.Rows.Count == 0)
                {
                    drExHentaiDetail = dataExHentaiDetail.NewRow();
                    drExHentaiDetail["PKEY"] = pKey;
                    drExHentaiDetail["SKEY"] = sKey;
                    // add placeholder fields that will be filled below
                    drExHentaiDetail["NAME_ENG"] = "";
                    drExHentaiDetail["NAME_UNI"] = "";
                    drExHentaiDetail["PAGE"] = 0;
                    drExHentaiDetail["PIC"] = "";
                    drExHentaiDetail["PIC_THUMB"] = "";
                    drExHentaiDetail["LANG"] = "";
                ...
                // If API returned metadata, apply it
                JArray resArr = res?["gmetadata"] as JArray;
                if (resArr != null && resArr.Count > 0)
                {
                    JObject resObj = resArr[0] as JObject;
                    if (resObj != null && !resObj.ContainsKey("error"))
                    {
                        // Map API fields
                        if (resObj.ContainsKey("title")) drExHentaiDetail["NAME_ENG"] = resObj["title"].ToString();
                        if (resObj.ContainsKey("title_jpn")) drExHentaiDetail["NAME_UNI"] = resObj["title_jpn"].ToString();
                        if (resObj.ContainsKey("thumb")) drExHentaiDetail["PIC_THUMB"] = resObj["thumb"].ToString();
                        if (resObj.ContainsKey("filecount")) drExHentaiDetail["PAGE"] = int.Parse(resObj["filecount"].ToString());
                        if (resObj.ContainsKey("posted"))
                        {
                            DateTime posted = new DateTime(1970, 1, 1, 0, 0, 0, 0, System.DateTimeKind.Utc)
                                .AddSeconds(long.Parse(resObj["posted"].ToString())).ToLocalTime();
                            drExHentai["RELEASE_TIME"] = posted.ToString("yyyy-MM-dd HH:mm:ss");
                        ...
                        // uploader etc can be stored if needed
                    }
                }

                // If API didn't provide all needed fields, or to get PIC (cover) and other site-only info, scrape thread page
                bool needMore = string.IsNullOrEmpty(drExHentaiDetail["NAME_ENG"].ToString()) ||
                                string.IsNullOrEmpty(drExHentaiDetail["PIC"].ToString()) ||
                                drExHentaiDetail["PAGE"] == null || drExHentaiDetail["PAGE"].ToString() == "0";

                if (needMore)
                {
                    try
                    {
                        // Scrape thread page for missing info using inlined scraper
                        BOOK_INFO scraped = GetBookInfoFromThread(url, "https://exhentai.org", Central.authUser);

                        if (!string.IsNullOrEmpty(scraped.eng_title)) drExHentaiDetail["NAME_ENG"] = scraped.eng_title;
                        if (!string.IsNullOrEmpty(scraped.uni_title)) drExHentaiDetail["NAME_UNI"] = scraped.uni_title;
                        if (scraped.total_page > 0) drExHentaiDetail["PAGE"] = scraped.total_page;
                        if (!string.IsNullOrEmpty(scraped.pic)) drExHentaiDetail["PIC"] = scraped.pic;
                        if (!string.IsNullOrEmpty(scraped.language)) drExHentaiDetail["LANG"] = scraped.language;
                        if (!string.IsNullOrEmpty(scraped.upload_time)) drExHentai["RELEASE_TIME"] = scraped.upload_time;
                        drExHentaiDetail["UPDATE_TIME"] = DateTime.Now;
                    }
                    catch (Exception ex)
                    {
                        Central.logger.WriteError($"Fallback scraping failed for {id}: {ex}");
                    }
                }

                // Finalize JSON status object for response
                jsonObject["TYPE"] = dataExHentaiDetail.Rows.Count == 0 ? "ADD" : "EDIT";
                jsonObject["PKEY"] = pKey;
                jsonObject["SKEY"] = sKey;
                jsonObject["NAME"] = drExHentaiDetail["NAME_UNI"] != null && drExHentaiDetail["NAME_UNI"].ToString() != "" ?
                                    drExHentaiDetail["NAME_UNI"].ToString() : drExHentaiDetail["NAME_ENG"].ToString();
            ...
            return Json(ViewData["DATA"], JsonRequestBehavior.AllowGet);
