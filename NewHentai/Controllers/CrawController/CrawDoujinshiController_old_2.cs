...
{
    public class CrawDoujinshiController_old_2 : Controller
    {
        ...
        public CrawDoujinshiController_old_2()
        {
...
SELECT O.* ,
 ...
 ORDER BY STATUS , CONVERT(DATE,UPDATE_TIME) , PKEY DESC";
        ...
        private BOOK_INFO GetBookInfoFromThread(THREAD_INFO threadInfo, AUTH auth, string domain)
        {
            BOOK_INFO book_info = new BOOK_INFO();
            try
            {
                string webHtml = FetchWeb(threadInfo.thread_url, auth);
                string pKey = threadInfo.thread_url.Replace(domain + "/g/", "").Split('/')[0];
                string sKey = threadInfo.thread_url.Replace(domain + "/g/", "").Split('/')[1];

                HtmlDocument document = new HtmlDocument();
                document.LoadHtml(webHtml);
                HtmlNode node = document.DocumentNode;

                // helper local function to select node
                Func<string, HtmlNode> GetNode = (path) =>
                {
                    try { return node.SelectSingleNode(path); } catch { return null; }
                };

                book_info = new BOOK_INFO(threadInfo.thread_url, pKey, sKey);
                ...
                if (n != null) book_info.total_page = int.TryParse(n.InnerHtml.Replace(" pages", ""), out int tp) ? tp : 0;
                ...
                // Cover (requires fetching gallery page that contains cover style)
                try
                {
                ...
                {
                    Central.logger.WriteError($"GetBookInfo cover fetch error: {ex}");
                }
